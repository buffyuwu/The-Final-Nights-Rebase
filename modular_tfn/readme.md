# The modularization handbook - TFN style, v0.3
...
```diff
- var/something = 1
+ var/something = 2 // TFN EDIT CHANGE - ORIGINAL: var/something = 1
```
...
```byond
var/something = 2 // TFN EDIT CHANGE - ORIGINAL: var/something = 4
```
...
And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `modular_tfn/modules/shuttle_toggle`
...
We will no longer have \_TFN map versions.
...
- **_Example:_** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `modular_tfn/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'modular_tfn/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'modular_tfn/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `modular_tfn/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`modular_tfn/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `modular_tfn/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes TFN edits are preferable. Just try to use your common sense about it.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. TFN edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  // TFN EDIT ADD START - SHUTTLE_TOGGLE - (Optional Reason/comment)
...
- **Removal:**
  ...
  /* // TFN EDIT REMOVAL START - SHUTTLE_TOGGLE - (Optional Reason/comment)
  ...
  *Please avoid this however, only to be done if there is no other option.
  ...
  ```byond
  /* // TFN EDIT REMOVAL START - SHUTTLE_TOGGLE - (Moved to modular_tfn/shuttle_toggle/randomverbs.dm)
...
- **Change:**
  ...
  ```byond
  if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE, SHUTTLE_DISABLED) // TFN EDIT CHANGE - ORIGINAL: if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE)
  ```
  ...
  ```byond
  /* // TFN EDIT REMOVAL START - Adds conditional
  	return 1
  */ // TFN EDIT REMOVAL
  // TFN EDIT ADD START - Adds conditional
  	if(!isnull(src))
  		return 1
  // TFN EDIT ADD END
...
That folder is **`code/__DEFINES/~TFN_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /modular_tfn/modules/example_module/code/disease_mob.dm
- Bad: /modular_tfn/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /modular_tfn/modules/example_module/icons/mining_righthand.dmi
- Bad: /modular_tfn/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for TFN Sector UIs.
...
You can do both `// TFN EDIT` and `/* TFN EDIT */`, though in some cases you may have to use one over the other.
...
<Button
	onClick={() => act('spin', { high_quality: true })}
	// TFN EDIT ADD START - another example, multiline changes
	icon="rat"
...
This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a TFN edit comment in a modular TGUI file.
